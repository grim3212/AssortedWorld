...
public class SpireFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        radius = 2;
        if (WorldCommonMod.COMMON_CONFIG.spireRadius.get() > 2) {
            radius = rand.nextInt(WorldCommonMod.COMMON_CONFIG.spireRadius.get() - 2);
        }
        height = (1 + rand.nextInt(WorldCommonMod.COMMON_CONFIG.spireHeight.get() * 2)) * 5;
        type = rand.nextInt(9);
        deathSpire = rand.nextFloat() < WorldCommonMod.COMMON_CONFIG.deathSpireChance.get();
